...
if ( ! function_exists( 'gh_exam_posted_by' ) ) :
	/**
	 * Prints HTML with meta information for the current author.
	 */
	function gh_exam_posted_by() {
		$byline = sprintf(
...
if ( ! function_exists( 'gh_exam_entry_footer' ) ) :
	...
	function gh_exam_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'gh-exam' ) );
			if ( $tags_list ) {
			...
			comments_popup_link(
				sprintf(
					wp_kses(
						...
						__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'gh-exam' ),
						array(
							'span' => array(
								'class' => array(),
							),
		...
		edit_post_link(
			sprintf(
				wp_kses(
					...
					__( 'Edit <span class="screen-reader-text">%s</span>', 'gh-exam' ),
					array(
						'span' => array(
							'class' => array(),
						),
...
if ( ! function_exists( 'gh_exam_post_thumbnail' ) ) :
	/**
	 ...
	 */
	function gh_exam_post_thumbnail() {
